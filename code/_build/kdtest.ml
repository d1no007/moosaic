...
(* Given a test point and a tree, finds the closest match to test point *)
  let rec nearest_neighbor (original : elt) (t : tree) : elt * float = 
    let rec traverse (o: elt) (current_best: elt) (k: tree) (d: float) (c: int) : elt * float =
        match k with 
        | Leaf -> (current_best, d)
        | Branch (l, hd, r) -> 
   ...
   * ran, the insert function worked completely *) 
  let test_insert ()  = 
    (*
    let e1 = ((3.0,1.0,4.0),"no") in 
    let e2 = ((2.0,3.0,7.0),"no") in
