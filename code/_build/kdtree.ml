...
type image = string ref 
  ...
  * returns the elt closest to the given elt in the tree *)
  let nearest_neighbor (x : elt) (t : tree) : elt * int  =
    let rec traverse (o: elt) (current_best: elt) (k: tree) (d: int) (c: int): elt * int =
        match k with 
        | Leaf -> (current_best, d)
        | Branch (l, hd, r) -> 
            let (current_best, d) = if distance hd o < d then (hd, distance hd o) else (current_best, d) in
  ...
  | Branch(_, hd, _) -> traverse x hd t (distance hd x) 0 
